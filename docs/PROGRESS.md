...
- **集成测试 demo 具身智能化 (2026-08-16)**: Layer 2 三条流水线从泛用场景改为机械臂主题——echo（关节位置回传）、multi-echo（关节位置 + 末端速度双路）、distance-guard（末端碰撞防护，0.15m 读数触发急停）。新增 `src/bin/distance-guard.rs` 节点二进制。三条流水线端到端验证 match:true。
